#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
REPO_DIR="$(cd "$ROOT_DIR/.." && pwd)"
SWIFT_BIN="${SWIFT_BIN:-/Users/willroper/Research/SWIFT/swiftsim/swift}"
MAKE_MOVIE="${MAKE_MOVIE:-0}"
EAGLE_FLAGS="--hydro --self-gravity --stars --cooling --star-formation --feedback"

cd "$REPO_DIR"

python -m swift_spiral_ics.cli.generate \
  --out-ics Runs/cheap_cpp/cheap_merger.hdf5 \
  --out-params Runs/cheap_cpp/cheap_merger.yml \
  --run-name cheap_merger \
  --snapshot-basename Runs/cheap_cpp/snapshot \
  --n-galaxies 2 \
  --xs -100 100 \
  --ys -10 10 \
  --zs 0 0 \
  --vxs 25 -25 \
  --vys 0 0 \
  --vzs 0 0 \
  --inclination-deg 0 0 \
  --dm-mass-msun 1e12 8e11 \
  --dm-part-mass-msun 5e9 \
  --star-mass-msun 6e10 4.8e10 \
  --bulge-fraction 0.1666666667 0.1666666667 \
  --star-part-mass-msun 1.6666666667e8 \
  --gas-mass-msun 1e10 8e9 \
  --gas-part-mass-msun 5e7 \
  --bulge-a-kpc 0.8 0.7155417528 \
  --stellar-disk-scale-length-kpc 3.5 3.1304951685 \
  --stellar-disk-scale-height-kpc 0.35 0.3130495168 \
  --gas-disk-scale-length-kpc 7.0 6.2609903370 \
  --gas-disk-scale-height-kpc 0.1 0.0894427191 \
  --box-kpc 1200 \
  --nR-grid 48 \
  --nz-grid 48 \
  --eps-grid 0.8 \
  --max-timestep-gyr 0.001 \
  --dt-min-gyr 1e-6 \
  --time-end-gyr 0.02 \
  --snapshot-dt-myr 10 \
  --arm-strength 0.15 \
  --arm-stream-frac 0.02 \
  --Q-star 2.0 \
  --Q-gas 1.5 \
  --bulge-rmax-scale 50

"$SWIFT_BIN" $EAGLE_FLAGS --threads=4 Runs/cheap_cpp/cheap_merger.yml

if [[ "$MAKE_MOVIE" == "1" ]]; then
  python create_movie.py Runs/cheap_cpp \
    --width-kpc 320 \
    --npix 360 \
    --fps 12 \
    --dpi 140 \
    --bitrate 2200 \
    --gas-vmin 5e4 \
    --gas-vmax 2e8 \
    --stars-vmin 5e4 \
    --stars-vmax 2e9 \
    --title-prefix "Cheap two-galaxy test"
fi
