...
def toomre_q_dispersion(
    ...
    """Return radial dispersion required for a target Toomre Q."""
    kappa = epicyclic_frequency(np.asarray(R, dtype=float), np.asarray(v_c, dtype=float))
    sigma = Q_target * np.pi * G.value * np.asarray(surface_density, dtype=float) / kappa
